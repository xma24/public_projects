...
class Models:
    class TorchModel:
        ...
        def get_model(num_classes):
            ...
            keyworlds = "resnet50"
            selected_listed_models = Models.TorchModel.get_timm_model_list(keyworlds)
        ...
        def check_weights_loading(to_test_model):
            ...
            if torch.cuda.is_available():
                input_batch = input_batch.to(Configs.TrainingConfig.device)
                to_test_model.to(Configs.TrainingConfig.device)
    ...
    class PytorchLightningDefault(pl.LightningModule):
        def __init__(self, logger=None):
            super(Models.PytorchLightningDefault, self).__init__()
            ...
            self.module_lr_dict = dict(backbone=Configs.NetConfig.backbone_lr)
            ...
            (
                ...
                self.test_precision,
                self.test_recall,
            ) = Models.PytorchLightningDefault.get_classification_metrics(
                Configs.DataConfig.num_classes, self.ignore
            )
        ...
        def train_dataloader(self):
            train_loader = Data.GetImagesCIFAR10.get_cifar10_dataloader(
                batch_size=Configs.TrainingConfig.batch_size,
                data_dir=Configs.TrainingConfig.data_dir,
                data_category="train",
            )
        ...
        def val_dataloader(self):
            val_loader = Data.GetImagesCIFAR10.get_cifar10_dataloader(
                batch_size=Configs.ValidationConfig.batch_size,
                data_dir=Configs.TrainingConfig.data_dir,
                data_category="test",
            )
        ...
        def configure_optimizers(self):
            ...
            if Configs.TrainingConfig.pretrained_weights:
                max_epochs = Configs.TrainingConfig.pretrained_weights_max_epoch
            else:
                max_epochs = Configs.TrainingConfig.max_epochs

            if Configs.TrainingConfig.scheduler == "cosineAnn":
                eta_min = Configs.TrainingConfig.eta_min
            ...
            elif Configs.TrainingConfig.scheduler == "CosineAnnealingLR":
                steps = 10
            ...
            elif Configs.TrainingConfig.scheduler == "step":
                step_size = int(Configs.TrainingConfig.step_ratio * max_epochs)
                gamma = Configs.TrainingConfig.gamma
            ...
            elif Configs.TrainingConfig.scheduler == "none":
                return optimizer
        ...
        def get_optim(self):
            lr = Configs.NetConfig.lr

            if Configs.TrainingConfig.pretrained_weights:
                lr = Configs.TrainingConfig.pre_lr

            if not hasattr(torch.optim, Configs.NetConfig.opt):
                print("Optimiser {} not supported".format(Configs.NetConfig.opt))
                raise NotImplementedError

            optim = getattr(torch.optim, Configs.NetConfig.opt)

            if Configs.TrainingConfig.strategy == "colossalai":
                from colossalai.nn.optimizer import HybridAdam

                optimizer = HybridAdam(self.parameters(), lr=lr)
            else:
                if Configs.TrainingConfig.single_lr:
                ...
                if Configs.NetConfig.opt == "Adam":
                ...
                elif Configs.NetConfig.opt == "Lamb":
                    weight_decay = 0.02
                    betas = (0.9, 0.999)
                ...
                elif Configs.NetConfig.opt == "AdamW":
                    eps = 1e-8
                    betas = (0.9, 0.999)
                    weight_decay = 0.05
                ...
                elif Configs.NetConfig.opt == "SGD":
                    momentum = 0.9
                    weight_decay = 0.0001
                    ...
                    optimizer = optim(grouped_parameters, lr=Configs.NetConfig.lr)
    ...
    class Model(PytorchLightningDefault):
        def __init__(self, logger=None):
            ...
            (
                ...
                init_classifier,
                classifier,
            ) = Models.TorchModel.get_model(num_classes=Configs.DataConfig.num_classes)
            ...
            # # init_model_bk = init_model_bk.to(Configs.TrainingConfig.device)
            ...
            # model = model.to(Configs.TrainingConfig.device)
        ...
        def validation_step(self, batch, batch_idx):
            ...
            self.log(
                "val_loss",
                val_loss,
                batch_size=Configs.ValidationConfig.batch_size * self.val_scales,
                **self.log_config_step,
            )
        ...
        def validation_epoch_end(self, outputs):
            ...
            if self.global_rank == 0:
                ...
                for i in range(Configs.DataConfig.num_classes):
                    self.console_logger.info(
                        "{0: <15}, acc: {1:.4f} | precision: {2:.4f} | recall: {3:.4f}".format(
                            Configs.DataConfig.classes[i],
...
class Settings:
    ...
    class ExprSetting(object):
        ...
        def logger_setting():
            ...
            if Configs.TrainingConfig.logger_name == "neptune":
                print("Not implemented")
                sys.exit()
            elif Configs.TrainingConfig.logger_name == "csv":
                print_logger = CSVLogger(Configs.DataConfig.logger_root)
            elif Configs.TrainingConfig.logger_name == "wandb":
                run_name = datetime.datetime.now(
                    tz=pytz.timezone("US/Central")
                ).strftime("%Y-%m-%d-%H-%M")
                print_logger = WandbLogger(
                    project=Configs.TrainingConfig.wandb_name,
        ...
        @staticmethod
        def gpu_setting():
            if Configs.TrainingConfig.training_mode == False:
                num_gpus = 1
            else:
                if isinstance(Configs.TrainingConfig.num_gpus, int):
                    num_gpus = Configs.TrainingConfig.num_gpus
                elif Configs.TrainingConfig.num_gpus == "autocount":
                    Configs.TrainingConfig.num_gpus = torch.cuda.device_count()
                    num_gpus = Configs.TrainingConfig.num_gpus
                else:
                    gpu_list = Configs.TrainingConfig.num_gpus.split(",")
        ...
        @staticmethod
        def dataset_setting():
            seed_everything(Configs.DataConfig.random_seed)
            if Configs.DataConfig.classes is None:
                Configs.DataConfig.classes = np.arange(Configs.DataConfig.num_classes)
        ...
        #     if Configs.DataConfig.dataloader_name == "dataloader":
        ...
        #     if Configs.NetConfig.model_name == "model":
        ...
        def pl_trainer_setting():
            if Configs.TrainingConfig.cpus:
                trainer = pl.Trainer(
                    num_nodes=Configs.TrainingConfig.num_nodes,
                    precision=Configs.TrainingConfig.precision,
                    accelerator="cpu",
                    strategy=DDPStrategy(find_unused_parameters=True),
                    # profiler="pytorch",
                    logger=Settings.ExprSetting.logger_setting()[1],
                    callbacks=[
                        Settings.ExprSetting.logger_setting()[0],
                        Settings.ExprSetting.checkpoint_setting(),
                    ],
                    log_every_n_steps=1,
                    check_val_every_n_epoch=Configs.ValidationConfig.val_interval,
                ...
                if (
                    Configs.TrainingConfig.strategy == "ddp"
                    or Configs.TrainingConfig.strategy == "ddp_sharded"
                ):
                    trainer = pl.Trainer(
                        devices=Settings.ExprSetting.gpu_setting(),
                        num_nodes=Configs.TrainingConfig.num_nodes,
                        precision=Configs.TrainingConfig.precision,
                        accelerator=Configs.TrainingConfig.accelerator,
                        strategy=Configs.TrainingConfig.strategy,
                        logger=Settings.ExprSetting.logger_setting()[1],
                        callbacks=[
                            Settings.ExprSetting.logger_setting()[0],
                            Settings.ExprSetting.checkpoint_setting(),
                        ...
                        # track_grad_norm=1,
                        # progress_bar_refresh_rate=Configs.TrainingConfig.progress_bar_refresh_rate,
                        # profiler="pytorch",  # "simple", "advanced","pytorch"
                    )
                    return trainer
                elif Configs.TrainingConfig.strategy == "deepspeed":
                    ...
                    return trainer
                elif Configs.TrainingConfig.strategy == "bagua":
                    bagua_strategy = BaguaStrategy(
                        algorithm=Configs.TrainingConfig.bagua_sub_strategy
    ...
    class GetImagesCIFAR10:
        ...
        def get_cifar10_dataloader(batch_size, data_dir, data_category="train"):
            ...
            loader = torch.utils.data.DataLoader(
                ...
                shuffle=use_shuffle,
                num_workers=Configs.DataConfig.workers,
                pin_memory=Configs.DataConfig.pin_memory,
            )
...
if __name__ == "__main__":
    ...
    pl_trainer = Settings.ExprSetting.pl_trainer_setting()

    if Configs.TrainingConfig.training_mode:
